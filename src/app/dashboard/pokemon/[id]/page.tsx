...
export async function generateStaticParams() {
  // Creates an array with 151 elements, each representing a Pokemon ID (from "1" to "151")
  const static151Pokemons = Array.from({ length: 151 }).map((v, i) => `${i + 1}`)

  // Returns an array of objects, each with an 'id' property corresponding to each Pokemon ID
...
export async function generateMetadata({ params }: PokemonPageProps): Promise<Metadata> {
  try {
    const { id, name } = await getOnePokemon(params.id)
